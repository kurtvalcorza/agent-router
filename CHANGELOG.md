...
- `docs/provider-instances.md`: design spec (unimplemented) for making `provider` a provider *instance* id with `kind` selecting the adapter, so two local servers can be addressed from one catalog.
